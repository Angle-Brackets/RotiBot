...
class Generate(commands.GroupCog, group_name = "generate"):
    ...
    async def _gen_image(self, interaction: discord.Interaction, prompt : str, style : Optional[str]):
        await interaction.response.defer()
         # Lets us have multiple people querying the bot at a time.
        buffer = await asyncio.to_thread(self.brain.generate_image, prompt, style)
