...
def _generate_help_embed(command = None):
    ...
    def _generate_pages(command_page = None):
        ...
        if command_page is None:
            ...
            embed.add_field(name="General Information", value=f"Uptime: ``{calculate_uptime()}``\nRoti currently has ``{len(help_information)}`` categories available, with a total of ``{sum(len(value["commands"]) for value in help_information.values())}`` commands available!", inline=False)
            ...
            embed.add_field(name="Additional Information", value="By @soupa. | [Github](https://github.com/Angle-Brackets/RotiBot)")
            return embed
